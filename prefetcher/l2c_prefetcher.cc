...
void CACHE::l2c_prefetcher_initialize()
{
	for(uint32_t index = 0; index < knob::l2c_prefetcher_types.size(); ++index)
	{
		...
		else if (!knob::l2c_prefetcher_types[index].compare("mlop_ppf"))
		{
			cout << "adding L2C_PREFETCHER: MLOP PPF" << endl;
			MLOP_PPF *pref_mlop_ppf = new MLOP_PPF(knob::l2c_prefetcher_types[index], this);
			prefetchers.push_back(pref_mlop_ppf);
		}
		else
		{
