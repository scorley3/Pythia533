...
#include "mlop.h"
using namespace mlop_ppf; 

extern char state_char[];
extern char getStateChar(MLOP_State);
extern std::string map_to_string(const std::vector<MLOP_State>&, const std::vector<int>&);
...
class MLOP_PPF : public Prefetcher
{
private:
    inline bool is_inside_zone(uint32_t zone_offset) { return (0 <= zone_offset && zone_offset < this->blocks_in_zone); }

    mlop_ppf::PERCEPTRON PERC; 
    mlop_ppf::PREFETCH_FILTER FILTER; 
    mlop_ppf::GLOBAL_REGISTER GHR;
...
public:
	MLOP_PPF(string type, CACHE *cache);
	~MLOP_PPF();
